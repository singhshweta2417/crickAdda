        ...
        body: ContainerConst(
            ...
            child: Column(
              children: [
                TextConst(
                  text:
                      "${args["title"]} ${AppLocalizations.of(context)!.youWouldLikeToUse}",
                  fontSize: Sizes.fontSizeOne,
                  alignment: FractionalOffset.centerLeft,
                ),
                Sizes.spaceHeight15,
                myInfoTextField(
                    profileProvider.updateContentCon, args["label"]),
                Sizes.spaceHeight20,
                ButtonConst(
                  onTap: () {
                    if (profileProvider.updateContentCon.text.isNotEmpty) {
                      profileProvider.updateProfileContactDetail(context, {
                        "updateData":
                            profileProvider.updateContentCon.text.trim(),
                        "updateType": args["updateType"]
                      });
                    } else {
                      Utils.showErrorMessage(context,
                          "Please enter ${args["updateType"]} to continue");
                      if (kDebugMode) {
                        print("please enter email to continue");
                      }
                    }
                  },
                  label: AppLocalizations.of(context)!.getOtp.toUpperCase(),
