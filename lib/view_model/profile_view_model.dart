...
import 'package:crickAdda/model/common_api_response.dart';
...
class ProfileViewModel extends ChangeNotifier {
  ...
  void updateToken(String token,context) {
    if (_userToken != token) {
      _userToken = token;
  ...
  Future<void> fetchUserProfile(context) async {
    ...
    try {
      _userProfile = await _profileApiService.fetchUserProfile(userId);
  ...
  Future<File?> _cropImage(File imageFile) async {
    final croppedFile = await ImageCropper().cropImage(
      ...
      aspectRatio: const CropAspectRatio(ratioX: 1, ratioY: 1), // example ratio
      uiSettings: [
        AndroidUiSettings(
